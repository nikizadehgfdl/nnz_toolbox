...
def getClocksFMSout(DIR, TAG='', loud=True, monthly=False):
    ...
    for file in FILES:
        ...
        yrlen = 4 if not monthly else 6
        modelYear.append(TAGlocal[:yrlen])
        MOM_date = []
        MOM_MaxCFL = []
        MOM_Trunc = [0]
        SIS_Trunc = [0]
...
def process_dora(doraID: str, db_path: str, replace_tables: bool = True, monthly: bool = False):
    if doralite is None:
    ...
    (
        ...
        RunDate,
        HiWaterMark,
    ) = getClocksFMSout(archDir, loud=False, monthly=monthly)

    if(monthly):
        YR = [float(y[:4])+float(y[4:6])/12 for y in YR if len(y) == 6]  # Extract year from YYYYMM
        logging.info('Monthly runs detected based on TAG length. Model years extracted are decimal years like %s', YR[2])
    ...
    experiment['NumberOfYears'] = len(YR) if not monthly else len(YR)/12.
    try:
    ...
    years = list(map(float, map(float, experiment.get('model_year', [])))) if len(experiment.get('model_year', [])) > 0 else []
    #print('Prepared years for database insertion:', years)
...
def main():
    ...
    parser.add_argument('--monthly', action='store_true', help='Indicate if the runs are monthly')
    args = parser.parse_args()

    for d in args.dora:
        process_dora(d, args.db, replace_tables=not args.append, monthly=args.monthly)
...
#example usage:
#python /nbhome/Niki.Zadeh/projects/nnz_toolbox/dora_utils/performancedb_dora_monthlyruns.py --dora cm5-15 --db ./CM5hires_performance.db
#INFO: Processing fms.out files in archDir /archive/oar.gfdl.cm5/frerts/FMS2025.03_cm5hires_08062025/cm5_hiresv2_am5f8d6r1_om5_b11_nonbous_piC/gfdl.ncrc6-intel23-prod-openmp/ for experiment cm5_hiresv2_am5f8d6r1_om5_b11_nonbous_piC
#expName                cm5_hiresv2_am5f8d6r1_om5_b11_nonbous_piC
#NumberOfYears                                                359
#TotalStorageSizeGB                                  73965.113506
#AnnualHistorySizeGB                                     60.16748
#HoursPerYear                                            1.511783
#platform                                           ncrc6-intel23
#RunDateSpan                                  20250929 - 20260225
